...
Pretty elegant once you wrap your head around it.
...
<Picture src={RateStuffOnline} formats={["webp", "avif"]} alt="Rate Stuff Online screenshot" />

This whole learning journey materialized into [rate-stuff.online](https://rate-stuff.online), a place to rate anything. Like literally *anything*. Concrete things like restaurants and albums. Abstract things like "the concept of Mondays" or "that feeling when you remember something embarrassing from ten years ago."
...
The TanStack pieces fit together seamlessly. Drizzle is a joy because it's type safe, has clean migrations, and doesn't hide the SQL from you. And running everything on Cloudflare's edge means the app responds fast from basically anywhere.
...
Then I tried **feature based** where you group everything by feature like "ratings" or "comments". That was better, but it still felt off. The boundaries weren't quite right.
...
const handleSubmit = (data) => {
  createRating({
    stuffId: "...",
    score: 8,
    content: "Pretty good tbh",
  });
};
```

There's no manual fetch and no REST endpoint boilerplate. You just define it, validate with Zod, and call it. Pretty neat!

## Real Time With PartyKit

I wanted votes and comments to update live. PartyKit makes WebSockets trivial because you just define a "party" (essentially a room), and it handles connection management, broadcasting, and state sync for you.

When someone votes, everyone looking at that rating sees the count change instantly. It makes the app feel alive instead of static.
...
Check out [rate-stuff.online](https://rate-stuff.online)!
